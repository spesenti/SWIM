...
sensitivity <- function(x, xCol = "all", wCol = "all", type = c("Gamma", "Kolmogorov", "Wasserstein", "all"), f = NULL){
  ...
  if(type == "Gamma" || type == "all"){
    sens_gamma_w <- function(z) apply(X = as.matrix(new_weights), MARGIN = 2, FUN = sensitivity_gamma, z = z)
  ...
  if(type == "Kolmogorov" || type == "all"){
    sens_kolmogorov_w <- function(z) apply(X = as.matrix(new_weights), MARGIN = 2, FUN = sensitivity_kolmogorov, z = z)
  ...
  if(type == "Wasserstein" || type == "all"){
    sens_wasser_w <- function(z) apply(X = as.matrix(new_weights), MARGIN = 2, FUN = sensitivity_wasserstein, z = z)
...
# reverse sensitivity Gamma
# comparison between input vectors for a given stress
sensitivity_gamma <- function(z, w){
  w <- as.numeric(w)
...
sensitivity_kolmogorov <- function(z, w){
  n <- length(z)
...
sensitivity_wasserstein <- function(z, w, p = 1){
  n <- length(z)
  x_sort <- sort(z)
  # xw_cdf[n] == n, as both ecdf are 1. 
