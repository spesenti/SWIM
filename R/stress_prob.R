 ...
 ## The data stem from continuously distributed random variables
  ...
  stress_prob <- function(x, prob, upper, lower= NULL, k = 1){
   ...
   if(is.unsorted(upper)) stop("'upper' has to be sorted")
   if(!is.null(lower) && is.unsorted(lower)) stop("'lower' has to be sorted")
   ...
   if((length(upper) != length(prob)) | (!is.null(lower) && (length(lower) != length(prob)))) stop("unequal length of lower, upper and prob")
    ...
    # calcualting the indicator function of{ a < x_data <= b}
    ...
    new_weights <- list(function(y) rn_temp(y) %*% as.matrix(prob_new / prob_old))
