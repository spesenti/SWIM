...
summary.SWIM <- function(x, xCol = "all", wCol = "all", base = FALSE){
  ...
  summary_w <- apply(X = as.matrix(new_weights), MARGIN = 2, FUN = summary_sub, x_data = x_data, cname = cname, base = base)

  if(base == TRUE){
  old_weights <- matrix(rep(1, length(x_data[,1])), ncol = 1)
  summary_base <- summary_sub(x_data = x_data, cname = cname, new_weights = old_weights)
...
# subfunction that calculates the summary when new_weights is a vector 
summary_sub <- function(x_data, cname, new_weights, base){

  temp <- function(y, w){
    temp_moments <- apply(X = y, FUN = moments, MARGIN = 2, w = w)
    return(temp_moments)
  }
...
# calculates weighted moments, mean, sd, skewness, excess kurtosis, quartiles
