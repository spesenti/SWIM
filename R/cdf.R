...
#' cdf(res1, xCol = 1, wCol = 1, grid = grid)
#' 
...
#' 
  ...
  cdf <- function(object, xCol = "all", wCol = 1, grid, base=FALSE){
    ...
    cdf <- .cdf(x = x_data, w = new_weights, grid = grid)
    colnames(cdf) <- cname
        
    if (base == TRUE){
      old_weights <- as.matrix(rep(1, length(x_data[,1])), ncol = 1)
      cdf_base <- .cdf(x = x_data, w = old_weights, grid = grid)
  ...
  .cdf <- function(x, w, grid){
    d <- ncol(x)
    t <- as.matrix(grid)
    
    res <- matrix(NA, ncol=d, nrow=ncol(t))
