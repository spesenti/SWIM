...
importance_rank <- function(x, xCol = "all", wCol = "all", type = c("Gamma", "Wasserstein", "all"), f = NULL){
  ...
  rank_w <- t(apply(X = sens_w[, 1:(length(sens_w)-2)], FUN = function(z) rank(z, ties.method = "min"), MARGIN = 1))
