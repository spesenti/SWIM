...
q_base <- cbind(as.numeric(quantile(x[, 1], probs = prob, type = 1)), 
            as.numeric(quantile(x[, 1], probs = prob, type = 1)))
q_stressed1 <- quantile_stressed(res1, probs = prob, xCol = "all", wCol = 1, type = "i/n")  
q_stressed2 <- quantile_stressed(res1, probs = prob, xCol = "all", wCol = 2, type = "i/n")  


test_that("base", {
  # test that the values for base is equal to the R quantile function 
  expect_equal(as.numeric(.VaR.2dim.1[, 3:4]), q_base)
  # test that the names are correct
  expect_equal(colnames(.VaR.2dim.1), c(names(x)[k], c("base","base")))
  # test that the values for base is equal to the R quantile function 
  expect_equal(as.numeric(.VaR.2dim.2[, 3:4]), q_base)
  # test that the names are correct
  expect_equal(colnames(.VaR.2dim.2), c(names(x)[k], c("base","base")))
